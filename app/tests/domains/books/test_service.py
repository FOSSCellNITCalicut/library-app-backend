...
from app.domains.books.service import BookNotFoundError, BookService, ServiceValidationError
...
class FakeRepository:
    ...
    async def browse_books(self, offset=0, limit=20, sort_by="title", sort_order="asc"):
        ...
        sort_key = {"title": "title", "published_year": "published_year", "authors": "authors", "publisher": "publisher"}.get(sort_by, "title")
...
class TestSearchByISBN:
    ...
    async def test_search_by_isbn_empty_raises(self, service):
        with pytest.raises(ServiceValidationError, match="ISBN must not be empty"):
...
class TestSearchBooks:
    ...
    async def test_search_empty_query_raises(self, service):
        with pytest.raises(ServiceValidationError, match="Search query must not be empty"):
    ...
    async def test_search_invalid_page(self, service):
        with pytest.raises(ServiceValidationError, match="Page must be >= 1"):
            await service.search_books(query="test", page=0)

    async def test_search_invalid_per_page(self, service):
        with pytest.raises(ServiceValidationError, match="per_page must be >= 1"):
...
class TestBookDetails:
    ...
    async def test_detail_has_availability(self, service):
        detail = await service.get_book_details(1)
        assert detail.availability_synced_at is not None
        assert detail.total_copies is not None
        assert detail.available_copies is not None
